...
// adding the  exact size constructor used by the optimized filters.cpp file
Matrix::Matrix(unsigned x, unsigned y, unsigned color_max_in)
    : R{ new unsigned char[x * y] }
    , G{ new unsigned char[x * y] }
    , B{ new unsigned char[x * y] }
    , x_size{ x }
    , y_size{ y }
    , color_max{ color_max_in }
{
}

...
class Matrix {
public:
    Matrix(unsigned char* R, unsigned char* G, unsigned char* B,
           unsigned x_size, unsigned y_size, unsigned color_max);
    Matrix();
    Matrix(unsigned dimension);
    // Added the exact size constructor used by optimized function in the filters.cpp file.
    Matrix(unsigned x_size, unsigned y_size, unsigned color_max);
